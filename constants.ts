import type { RoadmapSection, Language } from './types';
import { getTranslations } from './i18n';

const ROADMAP_EN: RoadmapSection[] = [
    {
    ...
    {
        title: "2. Reconnaissance & Enumeration",
        modules: [
             {
                id: 3,
    ...
    {
        title: "3. Web Application Hacking",
        modules: [
            {
                id: 5,
...
const ROADMAP_FA: RoadmapSection[] = [
    {
        title: "۱. مبانی اصلی",
        modules: [
            {
    ...
    {
        title: "۲. شناسایی و جمع‌آوری اطلاعات",
        modules: [
            {
                id: 3,
...
export const getRoadmap = (lang: Language): RoadmapSection[] => {
    const t = getTranslations(lang);
    const baseRoadmap = lang === 'fa' ? ROADMAP_FA : ROADMAP_EN;
    
    // Dynamically translate section titles
    return baseRoadmap.map((section, index) => {
        const sectionKey = `section${index + 1}Title` as keyof typeof t;
        return {
            ...section,
            title: t[sectionKey] || section.title
        };
    });
};
